...
ua.ie8 = ua.ie && typeof XDomainRequest !== 'undefined'; // initialize when DOM content is loaded
  ...
  if (ua.ie) {
    (function () {
      try {
        ...
        document.documentElement.doScroll('left'); // If we are in an iframe, the above does not work properly.
...
var cssHelper = function () {
  ...
  var objects = {
    stylesheet: function stylesheet(el) {
      ...
      var amqs = [],
          ...
          rs = [],
          rsw = [];
      var s = el.cssHelperText; // add attribute media queries
  ...
  var parseText = function parseText(el) {
    ...
    var stsh = o.stylesheet = objects.stylesheet(el); // collect media query lists

    var mqls = o.mediaQueryLists = stsh.getMediaQueryLists(); // collect all rules

    var ors = o.rules = stsh.getRules(); // collect all selectors
  ...
  var parseEmbedded = function parseEmbedded(el, s) {
    return; // This function doesn't work because of a bug in IE, where innerHTML gives us parsed css instead of raw literal.
...
domReady(function enableCssMediaQueries() {
  ...
  var createMeter = function createMeter() {
    ...
    document.body.appendChild(meter); // meter must have browser default font size of 16px
  ...
  var testMediaQueryList = function testMediaQueryList(mql, ts) {
    ...
    for (var i = 0; i < mqs.length; i++) {
      ...
      if (mqs[i].getExpressions().length === 0) {
        continue; // TODO: Browser check! Assuming old browsers do apply the bare media types, even in a list with media queries.
      }
  ...
  var testStylesheets = function testStylesheets(stshs) {
    ...
    if (ua.ie) {
      ...
      }, 0); // delay broadcast somewhat for IE
  ...
  var checkForResize = function checkForResize() {
    ...
    var cvph = cssHelper.getViewportHeight(); // determine scrollbar width in IE, see resizeHandler
    ...
    var resizeHandler = function resizeHandler() {
      ...
      var vph = cssHelper.getViewportHeight(); // check whether vp size has really changed, because IE also triggers resize event when body size changes
  ...
  docEl.style.marginLeft = '-32767px'; // make sure it comes back after a while
  ...
  return function () {
    if (!_nativeSupport()) {
      ...
      cssHelper.addListener('cssMediaQueriesTested', function () {
        ...
        }, 0); // remove this listener to prevent following execution
